...
│  └─────────┘   └─────────┘   └───────────┘              │
│                                                           │
│  Optional: Claude AI for caption mapping & narrative      │
└─────────────────────────────────────────────────────────┘
```
...
| **Testing**   | Vitest                   | Unit and integration tests (90 tests)        |
| **Optional**  | Anthropic SDK (Claude)   | AI-assisted caption mapping & narrative      |

### Third-Party APIs / External Services

**None required.** All processing happens locally on the server. The only optional external call is to the Anthropic API (Claude) for enhanced text understanding, and the app is fully functional without it.
...
> Without the API key, the app uses its built-in deterministic parser and rule-based analysis engine — everything works perfectly.
...
│       ├── llm/                  # Optional AI integration
│       │   ├── client.ts         # Anthropic SDK wrapper
│       │   ├── extractAssist.ts  # AI caption mapping
│       │   └── narrative.ts      # AI-written commentary
│       ├── domain/
...
When an Anthropic API key is configured, Claude assists in two narrow roles:
1. **Caption mapping** — identifies accounting concepts for captions the parser couldn't match
2. **Narrative** — writes a human-readable commentary from the computed figures

The AI never produces numbers and never sees the raw document. All figures come from the deterministic parser.
